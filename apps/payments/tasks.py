...
def send_payment_message(self, client_profile_id: int, text: str) -> None:
    ...
    try:
        ...
        logger.warning(f"Bot call failed for client_profile_id={client_profile_id}: {exc}")
        raise self.retry(exc=exc)


@shared_task(bind=True, max_retries=3, retry_backoff=30, retry_backoff_max=300)  # pyre-ignore[not-callable]
def send_client_request(self, coach_profile_id: int, client_profile_id: int, data: dict) -> None:
    async def _call_bot() -> None:
    ...
    try:
        ...
        logger.warning(
            f"Bot call failed for client_profile_id={client_profile_id}, coach_profile_id={coach_profile_id}: {exc}"
        )
        raise self.retry(exc=exc)
