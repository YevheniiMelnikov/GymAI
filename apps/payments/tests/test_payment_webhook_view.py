...
def test_webhook_view_valid(monkeypatch):
    data = base64.b64encode(
        json.dumps({"order_id": "1", "status": "success"}).encode()
    ).decode()
    request = SimpleNamespace(POST={"data": data, "signature": "sig"})

    monkeypatch.setattr(
        PaymentWebhookView, "_verify_signature", staticmethod(lambda d, s: True)
    )
    called = {}
    monkeypatch.setattr(
...
def test_webhook_view_bad_signature(monkeypatch):
    ...
    monkeypatch.setattr(
        PaymentWebhookView, "_verify_signature", staticmethod(lambda d, s: False)
    )
