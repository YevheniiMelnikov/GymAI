...
async def ask(data: AICoachRequest, request: Request) -> Program | Subscription | QAResponse | list[str] | None:
    ...
    if mode == CoachMode.ask_ai and not settings.AGENT_PYDANTICAI_ENABLED:
        try:
            responses = await KnowledgeBase.search(data.prompt or "", client_id=data.client_id)
        except Exception as e:  # pragma: no cover - log unexpected errors
            logger.exception(f"/ask legacy search failed: {e}")
    ...
    ctx: AskCtx = {
        ...
        "language": data.language or getattr(settings, "DEFAULT_LANG", "ru"),
