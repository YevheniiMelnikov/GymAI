...
async def update_profile_data(message: Message, state: FSMContext, status: str) -> None:
    ...
    try:
        ...
        if profile_data:
            await APIService.profile.edit_profile(profile.id, profile_data)

        if status == "client":
            Cache.client.set_client_data(profile.id, data)
            await APIService.profile.edit_client_profile(profile.id, data)
        else:
            ...
            await APIService.profile.edit_coach_profile(profile.id, data)
...
async def assign_coach(coach: Coach, client: Client) -> None:
    ...
    if client.id not in coach_clients:
        ...
        await APIService.profile.edit_profile(coach.id, {"assigned_to": coach_clients})

    coach_profile = await APIService.profile.get_coach_profile(coach.id)
    await APIService.profile.edit_client_profile(client.id, dict(coach=coach_profile.id))
    Cache.client.set_client_data(client.id, {"assigned_to": [coach.id]})
    await APIService.profile.edit_profile(client.id, {"assigned_to": [coach.id]})
...
async def get_user_profile(telegram_id: int) -> Profile | None:
    try:
        ...
        profile = await APIService.profile.get_profile_by_tg_id(telegram_id)
        if profile:
