...
from core.ai_coach import memify_run_at_key, memify_schedule_ttl, memify_scheduled_key
from core.utils.redis_lock import get_redis_client


class _MemifyTask(Protocol):
    def apply_async(
        self,
        *,
...
async def schedule_profile_memify(
    ...
    *,
    ...
    try:
        ...
        task_runner = cast(_MemifyTask, memify_profile_datasets)
        task = task_runner.apply_async(
