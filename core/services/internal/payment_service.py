...
class PaymentService(APIClient):
    ...
    def __init__(self, client: httpx.AsyncClient, settings, gateway: PaymentGateway | None = None) -> None:
    ...
    async def update_payment_status(self, order_id: str, status_: str, error: str = "") -> Payment | None:
        ...
        payment.status = PaymentStatus(status_)  # pyrefly: ignore[bad-assignment]
        payment.error = error
