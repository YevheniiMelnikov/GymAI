...
#
# @pytest.mark.asyncio
# async def test_log_in_successful(backend_service: BackendService) -> None:
#     response_mock = AsyncMock(status_code=200)
#     response_mock.json = MagicMock(return_value={"auth_token": "abc123"})
#     backend_service.client.request = AsyncMock(return_value=response_mock)
#
#     token = await backend_service.log_in("username", "password")
#
#     assert token == "abc123"
#     backend_service.client.request.assert_called_once_with(
#         "post",
#         f"{backend_service.backend_url}/auth/token/login/",
...
#
# @pytest.mark.asyncio
# async def test_log_in_failed(backend_service: BackendService) -> None:
#     json_mock = AsyncMock(return_value={"auth_token": "abc123"})
#     backend_service.client.request = AsyncMock(return_value=AsyncMock(status_code=400, json=json_mock))
#     token = await backend_service.log_in("username", "password")
...
#
# @pytest.mark.asyncio
# async def test_send_feedback_successful(backend_service: BackendService) -> None:
#     backend_service.client.request = AsyncMock(
#         return_value=AsyncMock(status_code=200, json=AsyncMock(return_value={"auth_token": "abc123"}))
#     )
#     success = await backend_service.send_feedback("test@example.com", "testuser", "Great service!")
#     assert success
#     backend_service.client.request.assert_called_once_with(
#         "post",
#         f"{backend_service.backend_url}/api/v1/send-feedback/",
