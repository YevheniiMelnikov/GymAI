...
async def test_send_feedback(api_service, status_code, expected_result):
    ...
    with patch.object(api_service.client, "request", side_effect=mock_request):
        result = await api_service.send_feedback("test@example.com", "testuser", "Great bot!")
...
async def test_send_welcome_email(api_service, status_code, expected_result):
    async def mock_request(method, url, json, headers):
        assert method == "post"
    ...
    with patch.object(api_service.client, "request", side_effect=mock_request):
        result = await api_service.send_welcome_email("test@example.com", "testuser")
        assert result == expected_result
...
async def test_api_request(api_service, response_kwargs, expected_status_code, expected_data):
    async def mock_request(method, url, json, headers):
        return Response(**response_kwargs)

    with patch.object(api_service.client, "request", side_effect=mock_request):
        status_code, data = await api_service._api_request("get", "http://example.com")
...
async def test_api_request_exceptions(api_service, exception, expected_message):
    async def mock_request(method, url, json, headers):
        raise exception

    with patch.object(api_service.client, "request", side_effect=mock_request):
        with pytest.raises(UserServiceError) as exc_info:
            await api_service._api_request("get", "http://example.com")
